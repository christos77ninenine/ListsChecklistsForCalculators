...
def sv(L,a):
 ...
 try:
  ...
  ts.store_list("C5NM",ne)
  ts.store_list("C5LS",se)
  ts.store_list("C5LD",te)
 except:pass
def ld():
 if not H:return None,0
 try:
  try:a=int(ts.recall_list("C5CF")[0])
  except:a=0
  st=ts.recall_list("C5LS");en=ts.recall_list("C5LD")
  if not st and not en:return None,0
  try:
   nm=[];cr=""
   for v in ts.recall_list("C5NM"):
