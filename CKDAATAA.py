...
def sv(L,a):
 ...
 try:
  ...
  ts.store_list("C4NM",ne)
  ts.store_list("C4LS",se)
  ts.store_list("C4LD",te)
 except:pass
def ld():
 if not H:return None,0
 try:
  try:a=int(ts.recall_list("C4CF")[0])
  except:a=0
  st=ts.recall_list("C4LS");en=ts.recall_list("C4LD")
  if not st and not en:return None,0
  try:
   nm=[];cr=""
   for v in ts.recall_list("C4NM"):
